...
## ADR-0005 first-boot bootstrap reconciliation — ✅ COMPLETE 2026-05-17

The last open ADR-vs-code drift before `v1.0.0`. Pre-change: ADR-0005
§"Bootstrap" promised autogen + stdout-print when
`RESTREAM_ADMIN_PASSWORD` was unset; shipping code wrote
`password_hash=NULL` and bricked the operator. Resolved as **option A**
(Rule №1 — implement the ADR rather than amend it). See
`docs/SESSION_HANDOFF.md` §"ADR-0005 first-boot bootstrap
reconciliation" for the full implementation + test + review +
Rule-№5-audit summary.

Schema-level changes: `SCHEMA_VERSION` bumped 1 → 2;
`users.password_hash` is now `NOT NULL` (enforced at schema, ORM, and
DTO levels). Per Rule №2 the upgrade procedure is export → reimport
(`docs/ops/upgrade.md`); pre-v1.0 has no production deployments to
migrate.

Audit log now distinguishes `admin_password_set` (source=env) vs
`admin_password_autogenerated` (source=autogen) vs
`admin_password_reset` — all carry `last4` only, never plaintext.
`_reset_admin_password` now emits its audit row (pre-change code
silently skipped despite ADR-0005 §"Password rotation" promising it).

Backend tests: **639/639 pass**; mypy --strict clean on `app/`.
Reviewer pass (`feature-dev:code-reviewer` + independent security
audit in parallel per Rule №4) cleared one M-1 finding (DTO type-lie)
and produced a PASS verdict from the security side. Rule №5 audit
grep-verified all 7 invariants.

**Next:** cut `v1.0.0` — release body MUST carry `[schema-bump]` per
`docs/ops/release-checklist.md:75-79`.
