...
Auth-flow events (login success/failure, password change, "log out of
all devices", admin-password reset, API token created/revoked) are
recorded in the append-only `audit_log` table. The complete event-type
inventory across the whole product is the single source of truth in
**ADR-0007 §"Audit log scope"** (auth + run lifecycle + secret
lifecycle + worker exceptional events + system maintenance lifecycle);
the original narrow enumeration that lived here was incomplete (it
predated the slice-8 SA-F4 amendment that documented the targets /
settings / credential CRUD rows). Look there, not here, when adding
a new event_type or auditing the catalogue.

### Bootstrap log-stream routing (slice 9 — SA-F8 ADR closure)

The autogenerated first-boot passphrase printed in §Autogen mechanics
is the *only* line the control plane intentionally writes to **stdout**
via raw `sys.stdout.write` + `flush()`. Everything else routes through
structlog, which is configured to write JSON to **stderr** (see
`app/logging_setup.py`). The split is load-bearing:

- The bootstrap banner is a human-readable secret meant for the operator
  who is watching the container's interactive console (`docker run`
  without `-d`, or `docker logs`) during first boot. It must NOT be
  shaped as a structured-log record because log-shipping pipelines
  (Loki, Filebeat, journald → Elasticsearch) routinely index JSON
  payloads — embedding the passphrase in a structured event would put
  it into search indexes the operator never expected.
- All operational logs (request lifecycle, audit-event echoes, error
  traces, retention-loop heartbeats) route to stderr via structlog so
  the operator's `docker logs --tail` and any sidecar log shipper see
  one consistent JSON stream. stdout is reserved for human-readable
  out-of-band signals (currently: only the bootstrap banner).
- A `CredentialRegistry` processor (`app/auth/credentials.py`)
  registers the autogen value AFTER the stdout print, so any
  accidental subsequent structlog mention is scrubbed. The single
  bootstrap line on stdout is the one place the value appears in
  cleartext, and it's gated on `RESTREAM_ADMIN_PASSWORD` being unset
  AND the DB being fresh.

The same routing convention applies to any future "operator pay attention
once" sentinel-bracketed line (e.g., a hypothetical "your TLS cert is
expiring in 14 days" first-boot warning): stdout + raw write + no
structlog. Anything that should land in the operator's log-shipping
pipeline must go through structlog and therefore stderr.
