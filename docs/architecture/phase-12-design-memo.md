...
- **S-13** `deployment.md` documents `RESTREAM_TRUSTED_PROXIES` and
  ...
  but safe)."*
- **S-14** No doc references a `.first-boot-password` *file*. Docs
  state shipping reality: `RESTREAM_ADMIN_PASSWORD` is OPTIONAL on
  first boot; if unset, the control plane autogenerates a 20-char
  passphrase and prints it to stdout under a
  `RESTREAM_PLUS FIRST BOOT` sentinel banner (ADR-0005 §"Bootstrap").
  Resolved 2026-05-17 — see §N.a.
...
### (a) ADR-0005 admin-bootstrap autogen drift — ✅ RESOLVED 2026-05-17

ADR-0005 §"Bootstrap" promised: *"if [RESTREAM_ADMIN_PASSWORD is]
absent, generates a random 20-char passphrase and prints it to stdout
exactly once."* Pre-resolution shipping code did NOT implement this —
if `RESTREAM_ADMIN_PASSWORD` was unset, the admin row got
`password_hash=NULL`, `first_run_complete=0`, and the operator
could not log in.

Resolved by implementing the ADR (option A — Rule №1: no quick-fix
ADR amendment to match broken code). Changes:

- `app/db/schema_init.py` — new `_resolve_bootstrap_password()` helper
  that returns `(SecretStr, source)`; autogen branch generates
  `secrets.token_urlsafe(15)` (20 chars), prints a sentinel-bracketed
  banner to stdout (NOT through structlog → stays out of the JSON
  log stream that ships to aggregators), and registers the value with
  the credential-redaction registry. `_seed_initial_rows` now always
  writes a non-NULL hash.
- `app/db/schema.sql` + `app/db/models.py` — `users.password_hash`
  tightened to `NOT NULL` (schema-level enforcement of the invariant).
  `SCHEMA_VERSION` bumped 1 → 2.
- Audit log distinguishes `admin_password_set` (source=`env`) from
  `admin_password_autogenerated` (source=`autogen`) from
  `admin_password_reset` (source=`reset`). All three carry `last4`
  only — never plaintext (project invariant).
- README + deployment.md document both paths; deployment.md warns
  unattended orchestrations (Compose/k8s) to set the env var explicitly.
- `tests/db/test_schema_init.py` covers autogen + idempotency +
  stdout-vs-stderr discipline + redaction registration + audit shape.
